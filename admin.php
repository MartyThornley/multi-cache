 ...
 */
	...
	function multi_cache_admin() {
		...
		<div class="error">
		    <?php _e('You must add to the file wp-config.php (at its beginning after the &lt;?php) the line of code: <code>define(\'WP_CACHE\', true);</code>.', 'multi-cache'); ?>
		</div>
		<?php } ?>
		...
		<?php
		    if ($error)
		        echo __('<p><strong>Options saved BUT not active because Hyper Cache was not able to update the config file (is it writable?).</strong></p>', 'multi-cache');
			
			if (!wp_mkdir_p( MULTI_CACHE_DIR ) )
		        echo __('<p><strong>Hyper Cache was not able to create the cache folder. Make it manually setting permissions to 777.</strong></p>', 'multi-cache');
		...
		<p class="submit">
			<?php if ( is_multisite() ) { ?>
			    <input class="button-primary" type="submit" name="multi_cache_site_clean" value="<?php _e('Clear cache for entire site', 'multi-cache'); ?>">
			<?php } else { ?>
			    <input class="button-primary" type="submit" name="multi_cache_blog_clean" value="<?php _e('Clear cache', 'multi-cache'); ?>">
			<?php }; ?>
		</p>
		
		<h3><?php _e('Cache status', 'multi-cache'); ?></h3>
		<table class="form-table">
		<tr valign="top">
		    <th><?php _e('Files in cache (valid and expired)', 'multi-cache'); ?></th>
		    <td>Do we need this? How to do it safely?</td>
		</tr>
		<tr valign="top">
		    <th><?php _e('Cleaning process', 'multi-cache'); ?></th>
		    <td>
		        <?php _e('Next run on: ', 'multi-cache'); ?>
		        <?php
		        $next_scheduled = wp_next_scheduled('multi_cache_clean');
		        ...
		        ?>
		        <div class="hints">
					<?php _e('The cleaning process runs hourly and it\'s ok to run it hourly: that grant you an efficient cache. If above there is not a valid next run time, wait 10 seconds and reenter this panel. If nothing change, try to deactivate and reactivate Hyper Cache.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		</table>
		...
		<h3><?php _e('Configuration for mobile devices', 'multi-cache'); ?></h3>
		<table class="form-table">
	
		<tr valign="top">
		    <th><?php _e('Detect mobile devices', 'multi-cache'); ?></th>
		    <td>
		        <input type="checkbox" name="options[mobile]" value="1" <?php echo $multi_cache_options['mobile']?'checked':''; ?>/>
		        <div class="hints">
		        <?php _e('When enabled mobile devices will be detected and the cached page stored under different name.', 'multi-cache'); ?>
		        <?php _e('This makes blogs with different themes for mobile devices to work correctly.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		
		<tr valign="top">
		    <th><?php _e('Mobile agent list', 'multi-cache'); ?></th>
		    <td>
		        ...
		        <div class="hints">
		        <?php _e('One per line mobile agents to check for when a page is requested.', 'multi-cache'); ?>
		        <?php _e('The mobile agent string is matched against the agent a device is sending to the server.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		</table>
		...
		<h3><?php _e('Compression', 'multi-cache'); ?></h3>
		
		<?php if (!function_exists('gzencode') || !function_exists('gzinflate')) { ?>
		
		<p><?php _e('Your hosting space has not the "gzencode" or "gzinflate" function, so no compression options are available.', 'multi-cache'); ?></p>
		
		<?php } else { ?>
		
		<table class="form-table">
		<tr valign="top">
		    <th><?php _e('Store compressed pages', 'multi-cache'); ?></th>
		    <td>
		        ...
		        <div class="hints">
		        <?php _e('Enable this option to minimize disk space usage and make sending of compressed pages possible with the option below.', 'multi-cache'); ?>
		        <?php _e('The cache will be a little less performant.', 'multi-cache'); ?>
		        <?php _e('Leave the options disabled if you note malfunctions, like blank pages.', 'multi-cache'); ?>
		        <br />
		        <?php _e('If you enable this option, the option below will be available as well.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		
		<tr valign="top">
		    <th><?php _e('Send compressed pages', 'multi-cache'); ?></th>
		    <td>
		        ...
		        <div class="hints">
		        <?php _e('When possible (i.e. if the browser accepts compression and the page was cached compressed) the page will be sent compressed to save bandwidth.', 'multi-cache'); ?>
		        <?php _e('Only the textual part of a page can be compressed, not images, so a photo
		        blog will consume a lot of bandwidth even with compression enabled.', 'multi-cache'); ?>
		        <?php _e('Leave the options disabled if you note malfunctions, like blank pages.', 'multi-cache'); ?>
		        <br />
		        <?php _e('If you enable this option, the option below will be available as well.', 'multi-cache'); ?>
		        </div>
		...
		<tr valign="top">
		    <th><?php _e('On-the-fly compression', 'multi-cache'); ?></th>
		    <td>
		        <input type="checkbox" name="options[gzip_on_the_fly]" value="1" <?php echo $multi_cache_options['gzip_on_the_fly']?'checked':''; ?> />
		        <div class="hints">
		        <?php _e('When possible (i.e. if the browser accepts compression) use on-the-fly compression to save bandwidth when sending pages which are not compressed.', 'multi-cache'); ?>
		        <?php _e('Serving of such pages will be a little less performant.', 'multi-cache'); ?>
		        <?php _e('Leave the options disabled if you note malfunctions, like blank pages.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		...
		<h3><?php _e('Advanced options', 'multi-cache'); ?></h3>
		
		<table class="form-table">
	
		<tr valign="top">
		    <th><?php _e('Disable Last-Modified header', 'multi-cache'); ?></th>
		    <td>
		        <input type="checkbox" name="options[lastmodified]" value="1" <?php echo $multi_cache_options['lastmodified']?'checked':''; ?>/>
		        <div class="hints">
		        <?php _e('Disable some HTTP headers (Last-Modified) which improve performances but some one is reporting they create problems which some hosting configurations.','multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		
		<tr valign="top">
		    <th><?php _e('Redirect caching', 'multi-cache'); ?></th>
		    <td>
		        <input type="checkbox" name="options[redirects]" value="1" <?php echo $multi_cache_options['redirects']?'checked':''; ?>/>
		        <br />
		        <?php _e('Cache WordPress redirects.', 'multi-cache'); ?>
		        <?php _e('WordPress sometime sends back redirects that can be cached to avoid further processing time.', 'multi-cache'); ?>
		    </td>
		</tr>
	
		
		<tr valign="top">
		    <th><?php _e('Allow browser to bypass cache', 'multi-cache'); ?></th>
		    <td>
		        <input type="checkbox" name="options[nocache]" value="1" <?php echo $multi_cache_options['nocache']?'checked':''; ?>/>
		        <div class="hints">
		        <?php _e('Do not use cache if browser sends no-cache header (e.g. on explicit page reload).','multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		</table>
		...
		<h3><?php _e('Filters', 'multi-cache'); ?></h3>
		<p>
		    <?php _e('Here you can: exclude pages and posts from the cache, specifying their address (URI); disable Hyper Cache for specific
		    User Agents (browsers, bot, mobile devices, ...); disable the cache for users that have specific cookies.', 'multi-cache'); ?>
		</p>
		
		<table class="form-table">
		<tr valign="top">
		    <th><?php _e('URI to reject', 'multi-cache'); ?></th>
		    <td>
		        <textarea wrap="off" rows="5" cols="70" name="options[reject]"><?php echo htmlspecialchars($multi_cache_options['reject']); ?></textarea>
		        <div class="hints">
		        <?php _e('Write one URI per line, each URI has to start with a slash.', 'multi-cache'); ?>
		        <?php _e('A specified URI will match the requested URI if the latter starts with the former.', 'multi-cache'); ?>
		        <?php _e('If you want to specify a stric matching, surround the URI with double quotes.', 'multi-cache'); ?>
		...
		<tr valign="top">
		    <th><?php _e('Agents to reject', 'multi-cache'); ?></th>
		    <td>
		        ...
		        <div class="hints">
		        <?php _e('Write one agent per line.', 'multi-cache'); ?>
		        <?php _e('A specified agent will match the client agent if the latter contains the former. The matching is case insensitive.', 'multi-cache'); ?>
		        </div>
		    </td>
		</tr>
		
		<tr valign="top">
		    <th><?php _e('Cookies matching', 'multi-cache'); ?></th>
		    <td>
		        ...
		        <div class="hints">
		        <?php _e('Write one cookie name per line.', 'multi-cache'); ?>
		        <?php _e('When a specified cookie will match one of the cookie names sent bby the client the cache stops.', 'multi-cache'); ?>
